...
public final class ParquetSettings {
    ...
    /**
     * Largest doc-values decode window this codec supports, in rows. Serves as both the default
     * ceiling and the upper bound accepted for either batch-size setting, so the ceiling can only
     * be lowered.
     * <p>
     * Matches the batch size the DataFusion engine uses elsewhere in this plugin
     * ({@code datafusion_query_config.rs}), so Arrow decodes here are sized like every other decode
     * in the process. Growing a window only amortises the fixed per-batch cost (one native call,
     * its out-parameter validation, one page lookup) over more rows; the Arrow decode itself is
     * per-row. That benefit flattens out well below this value, and a window is separately clamped
     * to the data page holding the requested row, so raising it has no measured upside. Mirrored by
     ...
     * index-scoped (see {@link #PAGE_ROW_LIMIT}). Dynamic updates apply to cursors opened after
     * the change; a cursor keeps the value it opened with for its lifetime.
     */
